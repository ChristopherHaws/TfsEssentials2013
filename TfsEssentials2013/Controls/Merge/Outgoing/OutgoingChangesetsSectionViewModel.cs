...
namespace Spiral.TfsEssentials.Controls.Merge.Outgoing
{
	internal class OutgoingChangesetsSectionViewModel : ChangesetsSectionViewModelBase
	{
		...
		private bool CanPush()
		{
			try
			{
				return this.Model != null && !this.Model.IsRepositoryOperationInProgress && (!this.Model.IsTfsOperationRunning && this.Model.Branch != null) && this.Model.Branch.HasUpstreamInfo && this.Model.Branch.OutgoingChangesets.Any();
			}
			catch (Exception ex)
			{
		...
		{
			if (this.Model != null && this.Model.Branch != null && this.Model.Branch.OutgoingChangesets != null)
			{
				this.ChangesetsItemsSource.Reset(this.Model.Branch.OutgoingChangesets);
			}
			else
			{
